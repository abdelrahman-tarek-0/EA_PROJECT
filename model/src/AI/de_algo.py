...
class DE:
    def __init__(self, *, data=[], gene_pool=None, Model=None, num_individuals=10, mutateWeight=0.8, crossoverRate=0.7, send_report=None, epochs=5, generations=100, layers=[], delay=1, input_dim=8, isParallel=False):
    ...
    def run(self):
        ...
        for j in range(self.generations):
            ...
            if self.isParallel:
                self.population = self.run_generation_parallel()
            else:
                self.population = self.run_generation()

            self.history.append([ind for ind in self.population])
        ...
        return (self.get_best_individual(), self.history)
